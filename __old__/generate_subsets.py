...
def generate():
    ...
    for folder in os.listdir("data/"):
        for file in os.listdir("data/" + folder):
            if os.path.isdir("data/" + folder + file): continue
            if not file.endswith("properties.yml"): continue
            
            path = os.path.join("data",folder,file.replace("properties.yml","subsets"))
            if not os.path.isdir(path):
                os.makedirs(path)

            existing = len(os.listdir(path))

            number_of_ccd = 18 #random.randint(15,30)
            cpt = existing
            for i in range(N - existing):
                ...
                block = yaml.safe_load(open(os.path.join("data",folder,file)))
